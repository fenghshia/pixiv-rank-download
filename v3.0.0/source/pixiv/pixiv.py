...
class pixiv:
    ...
    def __init__(self, check, counter):
        self.counter = counter
        self.storage_dir = check.get_config()
        self.counter.start_time = time()  # 爬虫开始时间
        ...
        # 设置cookies
        if check.check_cookie_exist():
            cookies = cookiejar_from_dict({"cookies": check.get_cookie()})
        ...
        for p in range(1, 11):  # 循环获取10页数据
            ...
            for data in r:  # 遍历数据
                if data['user_id'] not in self.black_user:  # 判断不在黑名单
                    ...
                    self.counter.ranknum += 1
                else:
                    self.counter.blacknum += 1
            self.counter.pagenum += 1

    def down_p0(self, data):
        if 'p0' in data['p_name']:  # 判断是否有p0
            ...
            while True:
                ...
                if not self.__download_init(data):
                    break
                p += 1
            self.counter.down_rank_num += 1
        else:
            self.__download_init(data)
            self.counter.down_rank_num += 1

    def __download_init(self, data):
        if not self.__exists(f'{self.config["warehouse"]}\\{data["dir_name"]}\\{data["p_name"]}'):  # 仓库是否有此图片
            if not self.__exists(f'{self.config["down_dir"]}\\{data["dir_name"]}\\{data["p_name"]}'):  # 下载路径是否有此图片
                if not self.__exists(f'{self.config["down_dir"]}\\{data["dir_name"]}'):  # 是否有画师的目录
                    if self.__create_dir(f'{self.config["down_dir"]}\\{data["dir_name"]}'):  # 创建目录
                        self.counter.create_dir_num += 1
    ...
    def __download(self, data):
        ...
        if img.status_code == 200:
            with open(f'{self.config["down_dir"]}\\{data["dir_name"]}\\{data["p_name"]}', 'wb') as f:
                f.write(img.content)  # 保存图片
                self.counter.down_p_num += 1
                return True
    ...
    def __get_black_user(self):
        with open(f'{self.storage_dir}\\pixiv_storage\\black_user.json', 'r') as f:
            try:
                return load(f)
            except Exception:
                with open(f'{self.storage_dir}\\pixiv_storage\\black_user.json', 'w') as fw:
                    dump([], fw, indent=4)
                    return []
